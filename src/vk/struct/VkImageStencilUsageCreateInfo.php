...
final class VkImageStencilUsageCreateInfo implements \JsonSerializable
{
    public function jsonSerialize(): array
    {
        return [
          ...
          "sType" => $this->getSType(),
          "pNext" => $this->getPNext(),
          "stencilUsage" => $this->getStencilUsage(),
        ];
    }
    ...
    {
        $self = new self( $vulkan->ffi->new('VkImageStencilUsageCreateInfo', false), $vulkan);
        if(!is_null($sType)) $self->setSType($sType);
        if(!is_null($pNext)) $self->setPNext($pNext);
        if(!is_null($stencilUsage)) $self->setStencilUsage($stencilUsage);
        return $self;
    }
