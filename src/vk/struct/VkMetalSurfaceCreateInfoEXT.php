...
final class VkMetalSurfaceCreateInfoEXT implements \JsonSerializable
{
    ...
    {
        $self = new self( $vulkan->ffi->new('VkMetalSurfaceCreateInfoEXT', false), $vulkan);
        if(!is_null($sType)) $self->setSType($sType);
        if(!is_null($pNext)) $self->setPNext($pNext);
        if(!is_null($flags)) $self->setFlags($flags);
        if(!is_null($pLayer)) $self->setPLayer($pLayer);
        return $self;
    }
